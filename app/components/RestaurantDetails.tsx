...
const RestaurantDetails = () => {
  ...
  // Find the restaurant by id from params
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <SafeAreaView className="bg-[#8B5CF6] rounded-b-[40px] pb-6 pt-12 px-5 relative overflow-hidden z-50">
        <SafeAreaView className="flex-row justify-between items-center z-10">
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        {/* Restaurant Details */}
        <SafeAreaView className="px-4 mt-6">
          <SafeAreaView className="flex-row items-center justify-between mb-2">
            <Text className="text-2xl font-bold text-black">{restaurant.name}</Text>
            <SafeAreaView className="bg-gray-200 px-3 py-1 rounded-full">
          ...
          {/* Chips */}
          <SafeAreaView className="flex-row flex-wrap gap-2 mb-4">
            {restaurant.chips.map((chip, i) => (
              <SafeAreaView key={i} className="bg-gray-100 px-3 py-2 rounded-full flex-row items-center">
                <SafeAreaView className="w-1 h-1 bg-gray-500 rounded-full mr-2" />
          ...
          {/* Deals & Menu Section */}
          <SafeAreaView className="flex-row gap-4 mb-6">
            <TouchableOpacity
              className="bg-[#8B5CF6] rounded-2xl p-4 flex-1"
              onPress={() => setModalVisible(true)}
            >
              <SafeAreaView className="flex-row items-center mb-2">
                <SafeAreaView className="w-6 h-6 bg-white rounded-full items-center justify-center mr-2">
                  <Text className="text-[#8B5CF6] text-xs font-bold">%</Text>
                </SafeAreaView>
                <Text className="text-white font-bold text-sm">SafeAreaView Deals & Discounts</Text>
              </SafeAreaView>
            </TouchableOpacity>

            <SafeAreaView className="bg-gray-50 rounded-2xl p-4 flex-1 items-center">
              <SafeAreaView className="flex-row items-center mb-2">
                <SafeAreaView className="w-6 h-6 bg-gray-300 rounded-full items-center justify-center mr-2">
          ...
          {/* Timings */}
          <SafeAreaView className="bg-gray-50 rounded-2xl p-4 mb-6">
            <SafeAreaView className="flex-row items-center mb-3">
              <SafeAreaView className="w-6 h-6 bg-gray-300 rounded-full items-center justify-center mr-2">
                <Text className="text-gray-600 text-xs font-bold">🕐</Text>
              </SafeAreaView>
            ...
            {Object.entries(restaurant.timings).map(([day, time]) => (
              <SafeAreaView key={day} className="flex-row justify-between py-1">
          ...
          {/* Confirm Button */}
          <ScrollView>
          <TouchableOpacity className="bg-[#8B5CF6] py-4 rounded-full items-center mb-8">
            <Text className="text-white font-bold text-lg">Confirm your Reservation</Text>
          </TouchableOpacity>
          </ScrollView>
        </SafeAreaView>
      </ScrollView>
    ...
    {/* Bottom Navigation */}
    <SafeAreaView className="absolute bottom-0 left-0 right-0 flex-row items-center justify-between bg-black rounded-t-3xl px-6 py-3 z-50">
      ...
      >
        <SafeAreaView className="flex-1 justify-end bg-black/50">
          <SafeAreaView className="bg-white rounded-t-3xl p-6 shadow-xl">
            <SafeAreaView className="w-12 h-1 bg-gray-300 rounded-full self-center mb-4" />
            <Text className="text-xl font-bold mb-6 text-center text-gray-800">Deals & Discounts</Text>
            {restaurant.deals.map((deal, i) => (
              <SafeAreaView key={i} className="bg-purple-50 border border-purple-200 p-4 rounded-xl mb-4">

...
export default function CustomerHome() {
  ...
  return (
    <View className="flex-1">
      ...
      {/* HUB Section */}
      <View className="flex-1 bg-white mt-6 rounded-t-3xl px-5 pb-32">
        <Text className="text-2xl font-extrabold text-gray-900 mb-4 text-center tracking-wide">
          HUB
        </Text>
        <View className="flex-row flex-wrap justify-between h-[2px]">
          {hubCards.map((card, idx) => (
            <View
              key={idx}
              className="w-[48%] bg-black rounded-2xl overflow-hidden shadow-lg mb-4"
              style={{ height: 236 }}
            >
              <Image
                source={card.img}
              ...
              <View className="p-3 flex-1 justify-between h-[2px]">
                <View>
                ...
                <TouchableOpacity
                  className="bg-[#8B5CF6] rounded-full py-2 px-4 self-start mt-3"
